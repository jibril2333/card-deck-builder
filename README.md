...
账号库为空时,首次连接数据库会自动建一个管理员,并把密码打印在启动日志里:
...
密码是随机生成的,不是固定默认值 —— 这个站点通常直接暴露在公网上,而
`admin/admin` 是被扫描得最多的一组凭据。想自己指定就设 `CDB_ADMIN_PASSWORD`
(此时日志里不会打印密码);完全不需要这个账号就设 `CDB_BOOTSTRAP_ADMIN=off`。

管理员权限有两个来源,任一满足即可:账号行上的 `is_admin`(上面这个账号就是这样
拿到的),以及 `CDB_ADMIN_EMAILS` 允许名单(逗号分隔,写在部署的环境变量里)。
两者都没有就没有管理员 —— 管理页面会整块消失,而不是对所有人开放。

再邀请别人注册,用宿主机上的 `npx tsx scripts/invite.ts` 生成一次性邀请码。
