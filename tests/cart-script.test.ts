...
/** The finished-message the importer passes; its wording is not what these
 *  tests are about. */
const DONE = MESSAGES.zh.deck.cartDone;
const HEADER = MESSAGES.zh.deck.cartHeader;
...
describe("paoCartScript", () => {
  it("carries the shop's product id and the quantity", () => {
    const s = paoCartScript(items, DONE, HEADER);
